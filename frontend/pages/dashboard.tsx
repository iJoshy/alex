...
export default function Dashboard() {
  ...
  const positionValues = Object.entries(positions)
    .flatMap(([, accountPositions]) =>
      accountPositions.map((position) => {
        const instrument = instruments[position.symbol];
        const price = Number(instrument?.current_price || 0);
        const value = Number(position.quantity) * price;
        return { symbol: position.symbol, value };
      })
    )
    .filter((item) => item.value > 0)
    .sort((a, b) => b.value - a.value);

  const topHolding = positionValues[0] || null;
  const concentrationPct =
    topHolding && totalValue > 0 ? Math.round((topHolding.value / totalValue) * 100) : 0;
  const concentrationLabel =
    concentrationPct >= 35 ? "High concentration" : concentrationPct >= 20 ? "Moderate concentration" : "Diversified";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="enterprise-hero p-6 mb-8">
          <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-2">
            Raenest US Shares Command Center
          </p>
          <h1 className="text-3xl font-bold text-dark mb-2">Investor Intelligence Dashboard</h1>
          <p className="text-sm text-gray-600 mb-4">
            Monitor portfolio health, concentration risk, and AI analysis freshness in one place.
          </p>
          <EnterpriseStatusStrip lastAnalysisDate={lastAnalysisDate} />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="surface-card p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-3">Top Holding Exposure</h3>
            {topHolding ? (
              <>
                <p className="text-2xl font-bold text-dark mb-2">{topHolding.symbol}</p>
                <p className="text-sm text-gray-600 mb-3">
                  ${topHolding.value.toLocaleString("en-US", { maximumFractionDigits: 2 })} market value
                </p>
                <span className={`status-chip ${concentrationClass}`}>
                  {concentrationPct}% of portfolio - {concentrationLabel}
                </span>
              </>
            ) : (
              <p className="text-sm text-gray-600">Add positions to see concentration insights.</p>
            )}
          </div>

          <div className="surface-card p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-3">Operational Readiness</h3>
            <div className="flex flex-wrap gap-2">
              <span className="status-chip status-chip-success">Trade Sync Ready</span>
              <span className="status-chip status-chip-info">AI Analysis Queue Ready</span>
              <span className="status-chip status-chip-warning">Human Advisor Review Recommended</span>
            </div>
            <p className="text-xs text-gray-500 mt-4">
              For production handover, keep runbooks and SLO dashboards current.
            </p>
          </div>
        </div>
